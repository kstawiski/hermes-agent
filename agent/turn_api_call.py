...
def perform_api_call(
    ...
    def _perform_api_call(next_api_kwargs):
        ...
        evidence = getattr(agent, "_oneshot_evidence", None)
        call_evidence = evidence.request(agent, next_api_kwargs) if evidence is not None else None
        if _use_streaming:
            value = agent._interruptible_streaming_api_call(
                next_api_kwargs, on_first_delta=_stop_spinner
            )
            if evidence is not None:
                evidence.response(call_evidence, value)
            return value
        from agent import relay_llm

        value = relay_llm.execute(
            next_api_kwargs,
